...
def roundSquare(width,edge=2e-9):
    step1 = step(edge) << width
    step2 = step(edge)
    return (step1 - step2) >> (width/2)


def nonroundSquare(width):
    return Waveform(bounds=(-0.5 * width, 0.5 * width, +np.inf),
                    seq=(_zero, _one, _zero))


def square(width,edge=0e-9):
    if edge == 0:
        return nonroundSquare(width)
    else: 
        return roundSquare(width,edge)
...
def mixing(pulse,
    ...
    return Iout*(1+ampDiff[0])+ Offset[0], Qout*(ampDiff[1]+1)+ Offset[1]
